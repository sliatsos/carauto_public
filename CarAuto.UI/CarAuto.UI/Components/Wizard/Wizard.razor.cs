...
namespace CarAuto.UI.Components.Wizard
{
    /// <summary>
    /// Wizard Component
    /// </summary>
    public partial class Wizard
    {
        ...
        /// </summary>
        [Parameter]
        public int ActiveStepIx { 
            get => _activeStepIx; 
            set
            {
                _activeStepIx = value;
                if (ActiveStepIx <= Steps.Count - 1)
                    SetActiveStepIndex(Steps[ActiveStepIx]);
            }
        }
        ...
        protected internal void SetActiveStepIndex(WizardStep step)
        {
            ActiveStep = step ?? throw new ArgumentNullException(nameof(step));
            if (ActiveStepIx == Steps.Count - 1)
                IsLastStep = true;
